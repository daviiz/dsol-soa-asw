...
public class Decoy extends EventProducer implements EventListenerInterface,Locatable{
    ...
    private synchronized void next() throws RemoteException, SimRuntimeException, NamingException
    {
    	...
    	if(!_mdata.status) {
        	...
        	this._mdata.destination = SimUtil.nextPoint(this._mdata.origin.x, this._mdata.origin.y, lastThreat.x, lastThreat.y, 2.0,false);
        }
        ...
        super.fireTimedEvent(DECOY_LOCATION_MSG, new EntityMSG(_mdata.name,_mdata.belong,_mdata.status,this._mdata.origin.x,this._mdata.origin.y),this.simulator.getSimTime().plus(2.0));
