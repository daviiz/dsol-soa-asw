...
public class Visual2dRender extends Renderable2D<Locatable>
{
     ...
     */
    public Visual2dRender(final Locatable source, final SimulatorInterface.TimeDouble simulator,Color _color,int _detectRange,LineData _target)
    ...
    public void paint(final Graphics2D graphics, final ImageObserver observer)
    {
        ...
        if(this.data.lineData != null) {
    		graphics.setColor(this.data.color);
    		int x=0,y=0;
    			x = this.data.lineData.x1>this.data.lineData.x2 ? (-1)*(Math.abs(this.data.lineData.x1-this.data.lineData.x2)):(Math.abs(this.data.lineData.x1-this.data.lineData.x2));
    			y = this.data.lineData.y1>this.data.lineData.y2 ? (Math.abs(this.data.lineData.y1-this.data.lineData.y2)):(-1)*(Math.abs(this.data.lineData.y1-this.data.lineData.y2));;
